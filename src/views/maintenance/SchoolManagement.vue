    ...
    <el-card>
      ...
      <el-table :data="filteredSchools" style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button size="small" type="primary" @click="editSchool(row)">
              编辑
            </el-button>
    ...
    >
      ...
      >
        <el-form-item label="学校ID" prop="id">
          <el-input v-model="schoolForm.id" placeholder="请输入学校ID">
            <template #append>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="submitSchool" :loading="submitLoading">
            {{ isEdit ? '更新' : '添加' }}
          </el-button>
        </span>
      </template>
...
const loadUnregisteredSchool = async () => {
  ...
  try {
    const response = await schoolAPI.getUnregisteredSchool(schoolForm.value.id)
    if (response.status === 200) {
...
const loadAvailableAdmins = async () => {
  try {
    const response = await schoolAPI.getAvailableAdmins()
    if (response.status === 200) {
      availableAdmins.value = response.data.users || []
    }
  } catch (error) {
...
const toggleStatus = async (school) => {
  try {
    ...
    const response = await schoolAPI.updateSchool(school.id, {
      status: newStatus
    })
...
const syncSchool = async (school) => {
  try {
    const response = await schoolAPI.syncSchoolData(school.schoolId)
    if (response.status === 200) {
...
const submitSchool = async () => {
  ...
  await schoolFormRef.value.validate(async (valid) => {
    ...
    try {
      const schoolData = {
        school_id: schoolForm.value.id,
        admin_id: schoolForm.value.adminId,
        is_enabled: schoolForm.value.status === 'active'
      }

      let response
      if (isEdit.value) {
        response = await schoolAPI.updateSchool(schoolForm.value.id, schoolData)
      } else {
