  ...
  <div class="student-management">
    <el-card>
      ...
      <el-table :data="filteredStudents" style="width: 100%" v-loading="loading">
        <el-table-column prop="user_id" label="学生ID" width="200" show-overflow-tooltip />
...
const filteredClasses = computed(() => {
  ...
  const classMap = {}
  students.value.forEach(student => {
    if (student.school_info && student.school_info.id === selectedSchool.value) {
...
const filteredStudents = computed(() => {
  ...
  if (selectedSchool.value) {
    result = result.filter(student => student.school_info.id === selectedSchool.value)
  }

  if (selectedClass.value) {
    result = result.filter(student => student.class_info.id === selectedClass.value)
  }
...
const submitStudent = async () => {
  ...
  await studentFormRef.value.validate(async (valid) => {
    ...
    try {
      const school = schools.value.find(s => s.id === studentForm.value.schoolId)
      const classItem = classes.value.find(c => c.id === studentForm.value.classId)
